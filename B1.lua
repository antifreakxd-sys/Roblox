...
ScreenGui.Name = "AntiFreak_V20_InfinityGod"
ScreenGui.ResetOnSpawn = false
ScreenGui.Parent = LocalPlayer:WaitForChild("PlayerGui")

-- Кнопка-шестерёнка ⚙️ с неоновой обводкой
...
MainMenu.Size = UDim2.new(0, 290, 0, 110) 
...
-- Неоновая светящаяся обводка меню
...
Title.Text = "🌌 ANTIFREAK INFINITY v20"
...
FlyToggleBtn.Size = UDim2.new(0, 200, 1, 0)
FlyToggleBtn.BackgroundColor3 = Color3.fromRGB(24, 20, 32)
FlyToggleBtn.Text = "🚀 LAUNCH GOD FLY"
...
-- Стрелочка настроек скорости
local ArrowBtn = Instance.new("TextButton")
ArrowBtn.Size = UDim2.new(0, 45, 1, 0)
ArrowBtn.Position = UDim2.new(0, 205, 0, 0)
...
SliderText.Text = "⚡ SPEED ACCELERATION: 220"
...
-- Закрашенная часть слайдера (Неоновый след)
...
-- Логика плавного перетаскивания окон
...
-- Микро-анимации наведения (Hover Effects)
...
-- Логика сворачивания/разворачивания главного VIP-Меню
local menuOpen = false
local arrowExpanded = false
local normalSize = UDim2.new(0, 290, 0, 110)
local expandedSize = UDim2.new(0, 290, 0, 170)
...
UI.ToggleButton.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(UI.MainMenu, tInfo, {BackgroundTransparency = menuOpen and 0 or 1, Size = menuOpen and currentTargetSize or UDim2.new(0, 290, 0, 0)}):Play()
...
-- Математика и плавная неоновая заливка ползунка скорости
...
local function updateSlider(input)
    ...
    -- Плавное изменение ширины неонового следа
    ...
    UI.SliderText.Text = "⚡ SPEED ACCELERATION: " .. tostring(finalSpeed)
end
...
-- Переключатель активного статуса
UI.FlyToggleBtn.MouseButton1Click:Connect(function()
    if getgenv().TreasureAutoFarm.Mode == "Fly" then 
        ...
        UI.FlyToggleBtn.Text = "🚀 LAUNCH GOD FLY"
        ...
        UI.FlyToggleBtn.Text = "⚡ GOD FLY: ACTIVE" 
...
-- ТОТАЛЬНЫЙ CFRAME NOCLIP (Сброс векторов сил аниматора каждую миллисекунду)
...
-- ДВИЖЕНИЕ МАТЕМАТИЧЕСКИМИ ВЕКТОРАМИ С ОБХОДОМ СТРИМИНГА ROBLOX
local function godFlyTo(targetPos, stageIndex)
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end

    -- Привязываем фокус камеры к цели, чтобы Roblox принудительно прогрузил чанк уровня заранее
    ...
    if hrp and getgenv().TreasureAutoFarm.Mode == "Fly" then 
        ...
        hrp.Anchored = true -- Замораживаем физику
        
        -- Сверхпробитие чекпоинта: шлем серверу 10 пакетов касания стадии за один кадр
        if stageIndex then
            pcall(function()
        ...
        -- Оптимальная задержка фиксации под скорость 1500
...
task.spawn(function()
    while true do
        ...
        if getgenv().TreasureAutoFarm.Mode == "Fly" and hrp then
            pcall(function()
                ...
                -- Скоростной пролет стадий 2 - 10 с фокусом чанков
                for i = 2, 10 do
                    ...
                    godFlyTo(centerPos, i)
                    
                    -- Двойной зигзаг для зачета скрытых триггеров монет
                    if i == 3 or i == 6 or i == 9 or i == 10 then
                        godFlyTo(centerPos + Vector3.new(20, 0, 0), false)
                        godFlyTo(centerPos - Vector3.new(20, 0, 0), false)
                        godFlyTo(centerPos, i)
                    else
                        task.wait(0.05)
                ...
                if getgenv().TreasureAutoFarm.Mode == "Fly" and normalStages:FindFirstChild("TheEnd") then
                    ...
                    if trigger then 
                        ...
                        hrp.CFrame = CFrame.new(-50, -15, -9200) end
                    task.wait(getgenv().TreasureAutoFarm.WaitAtChest)
                end

                -- Моментальное авто-убийство и моментальный респавн для повтора круга
                if getgenv().TreasureAutoFarm.Mode == "Fly" then
                    ...
                    repeat task.wait(0.02) until respawned
                    conn:Disconnect()
                end
            end)
        end
