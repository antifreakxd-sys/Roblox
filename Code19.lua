-- ГЛОБАЛЬНЫЕ НАСТРОЙКИ С УЛЬТРА-СКОРОСТЬЮ
getgenv().TreasureAutoFarm = {
    Enabled = false,
    FlySpeed = 220,        -- Диапазон ползунка теперь: 220 - 1500
...
ScreenGui.Name = "AntiFreak_Hyper_v4"
...
-- СТИЛИЗАЦИЯ КНОПКИ (Шестеренка ⚙️ + Тёмный премиум стиль)
...
-- ЛОГИКА ПОЛЗУНКА СКОРОСТИ (220 - 1500)
...
-- СТАБИЛЬНЫЙ NOCLIP И УДАЛЕНИЕ РЕГИСТРАТОРОВ СМЕРТИ
...
-- ВЫСОКОСКОРОСТНОЙ ДВИЖОК С СУБ-ШАГАМИ (ДЛЯ СТАБИЛЬНОСТИ НА 1500)
local function hyperFlyTo(targetPos)
    ...
    while getgenv().TreasureAutoFarm.Enabled and (hrp.Position - targetPos).Magnitude > 5 do
        ...
        -- На сверхвысокой скорости делим шаг кадра на под-шаги, чтобы хитбокс не перелетал триггер
        local steps = (speedThisFrame > 1000) and 4 or 1
        local stepDistance = (speedThisFrame * delta) / steps
        ...
        for s = 1, steps do
            ...
            hrp.Velocity = Vector3.new(0, 0, 0) -- Мгновенное гашение инерции
...
-- ИДЕАЛЬНЫЙ АЛГОРИТМ ДЛЯ СКОРОСТЕЙ ДО 1500
task.spawn(function()
    while true do
        task.wait(0.5)
        if getgenv().TreasureAutoFarm.Enabled then
            pcall(function()
                ...
                for i = 2, 10 do
                    ...
                    -- СБОР ПРОБЛЕМНЫХ И УЗКИХ СТАДИЙ НА БЕШЕНОЙ СКОРОСТИ
                    if i == 3 or i == 6 or i == 9 or i == 10 then
                        hyperFlyTo(centerPos)
                        
                        -- Сверхбыстрое «прочёсывание» площади на микро-шагах для 100% фиксации
                        hyperFlyTo(centerPos + Vector3.new(12, 0, 0))
                        hyperFlyTo(centerPos - Vector3.new(12, 0, 0))
                        hyperFlyTo(centerPos)
                        
                        -- Пинг-компенсация в зависимости от скорости ползунка
                        local checkDelay = (getgenv().TreasureAutoFarm.FlySpeed > 1000) and 0.75 or 0.55
                        task.wait(checkDelay)
                    else
                        -- Обычные стадии: Смещение вправо (уклонение), затем центр
                        hyperFlyTo(centerPos + sideOffset)
                        hyperFlyTo(centerPos)
