...
-- ЖЕСТКИЙ NOCLIP + СБРОС СЕТЕВОЙ ФИЗИКИ (Полная блокировка античита)
...
-- Функция протаскивания персонажа ПОД картой (Никаких вылетов за карту)
local function safeMoveUnderground(targetPos, stageIndex)
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end

    -- Опускаем координату цели под землю на безопасную глубину (Y = -15)
    local undergroundTarget = Vector3.new(targetPos.X, -15, targetPos.Z)

    while getgenv().TreasureAutoFarm.Mode == "Fly" and (hrp.Position - undergroundTarget).Magnitude > 15 do
        if not LocalPlayer.Character or not hrp.Parent then break end
        local dir = (undergroundTarget - hrp.Position).Unit
        local step = getgenv().TreasureAutoFarm.FlySpeed * RunService.Heartbeat:Wait()
        if step > (undergroundTarget - hrp.Position).Magnitude then step = (undergroundTarget - hrp.Position).Magnitude end
        
        -- Перемещение строго через CFrame, физика игры отключена
        hrp.CFrame = CFrame.new(hrp.Position + dir * step, undergroundTarget)
    end
    
    if hrp and getgenv().TreasureAutoFarm.Mode == "Fly" then 
        hrp.CFrame = CFrame.new(undergroundTarget)
        
        -- Инжект пакета касания стадии прямо из под земли
        if stageIndex then
            pcall(function()
            ...
            task.wait(0.15) -- Кратчайшая задержка, сервер успевает выдать золото
        end
    end
end
    ...
    while true do
        ...
        if getgenv().TreasureAutoFarm.Mode == "Fly" and hrp and hum and hum.Health > 0 then
            pcall(function()
                ...
                -- Ныряем под землю на старте
                hrp.CFrame = CFrame.new(-50, -15, -1025)
                task.wait(0.2)

                -- Чистый подземный проход по цепочке стадий без лагов и застреваний
                for i = 1, 10 do
                    ...
                    if stage and stage:FindFirstChild("DarknessPart") then 
                        ...
                        stagePos = Vector3.new(-50, -15, -1025 + (i * -775))
                    end
                    
                    safeMoveUnderground(stagePos, i)
                end
                
                -- Влёт в финальный сундук (поднимаемся только в самом конце)
                ...
                -- Моментальный ресет для мгновенного перезапуска круга
                if getgenv().TreasureAutoFarm.Mode == "Fly" then
