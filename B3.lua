...
ScreenGui.Name = "AntiFreak_V16_Core"
...
-- НАДЕЖНЫЙ NOCLIP + СБРОС ИНЕРЦИИ (Исключает любые вылеты за карту на 1500)
...
-- ДВИЖЕНИЕ ТОЛЬКО ЧЕРЕЗ ЧИСТЫЙ CFRAME (Убирает физические баги)
local function safeFlyTo(targetPos)
    ...
    while getgenv().TreasureAutoFarm.Mode == "Fly" and (hrp.Position - targetPos).Magnitude > 15 do
        ...
        -- Смещение CFrame напрямую, без воздействия физического движка Roblox
        hrp.CFrame = CFrame.new(hrp.Position + dir * step, targetPos)
    end
    
    if hrp and getgenv().TreasureAutoFarm.Mode == "Fly" then 
        hrp.CFrame = CFrame.new(targetPos)
        -- АНКЕРНЫЙ ФИКСАТОР: Намертво прибиваем тело в триггере, чтобы сервер успел выдать золото
        hrp.Anchored = true
        local anchorTime = (getgenv().TreasureAutoFarm.FlySpeed >= 1200) and 0.5 or 0.25
        task.wait(anchorTime)
        hrp.Anchored = false
    end
end

-- СТУПЕНЧАТЫЙ ТЕЛЕПОРТ С ОЖИДАНИЕМ ЗАГРУЗКИ КАРТЫ
local function safeTPTo(targetPos)
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
    ...
    local steps = math.ceil(distance / 75) -- Мелкие шаги, чтобы Roblox успевал прогружать триггеры стадий
    ...
    if hrp and getgenv().TreasureAutoFarm.Mode == "TP" then
        hrp.CFrame = CFrame.new(targetPos)
        hrp.Anchored = true
        task.wait(0.4) -- Даем серверу время обработать ТП
        hrp.Anchored = false
    end
end
        ...
        if getgenv().TreasureAutoFarm.Mode ~= "None" and hrp then
            pcall(function()
                ...
                if getgenv().TreasureAutoFarm.Mode == "Fly" or getgenv().TreasureAutoFarm.Mode == "TP" then
                    -- Разблокировка спавна и старт первой стадии
                    ...
                    for i = 2, 10 do
                        ...
                        if getgenv().TreasureAutoFarm.Mode == "Fly" then
                            ...
                            if i == 3 or i == 6 or i == 9 or i == 10 then
                                -- Тройной зигзаг для 100% зачета зон
                                safeFlyTo(centerPos + Vector3.new(20, 0, 0))
                                safeFlyTo(centerPos - Vector3.new(20, 0, 0))
                                safeFlyTo(centerPos)
                            else
                                task.wait(0.2)
                    ...
                    -- Ресет персонажа для повторения цикла
                ...
                -- ТОТ САМЫЙ КУСТОВОЙ АФК-ГЛИТЧ (БЕЗ СМЕРТЕЙ И ПЕРЕЗАПУСКОВ)
                elseif getgenv().TreasureAutoFarm.Mode == "AFK" then
                    -- Мгновенно ставим тебя за водопад/кусты первой зоны
                    ...
                    -- Замораживаем физику и включаем микро-качание для обхода анти-афк системы игры
                    hrp.Anchored = true
                    local direction = 1
                    while getgenv().TreasureAutoFarm.Mode == "AFK" do
                        -- Микро-сдвиг на уровне CFrame (сервер бесконечно начисляет золото)
