...
ScreenGui.Name = "AntiFreak_V13_Core"
...
-- Noclip + Жесткое гашение сил (теперь торс НЕ улетает вперед скрипта)
...
-- Исправленный Fly: полная остановка и жесткий фиксатор (якорь) в контрольных точках
local function safeFlyTo(targetPos)
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end

    while getgenv().TreasureAutoFarm.Mode == "Fly" and (hrp.Position - targetPos).Magnitude > 10 do
    ...
    if hrp and getgenv().TreasureAutoFarm.Mode == "Fly" then 
        -- Якорим деталь на доли секунды, чтобы физический движок Roblox применил CanCollide триггеров
        hrp.Anchored = true
...
-- Пошаговый телепорт (решает проблему с пропуском негруженых стадий)
local function safeTPTo(targetPos)
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
    
    -- Плавный ТП «кликами», чтобы сервер успевал за подгрузкой чанков стадий
    local currentPos = hrp.Position
    local distance = (targetPos - currentPos).Magnitude
    local steps = math.ceil(distance / 150) -- режем путь на отрезки по 150 единиц
    ...
    while true do
        ...
        if getgenv().TreasureAutoFarm.Mode ~= "None" and hrp then
            pcall(function()
                ...
                -- ОБЫЧНЫЕ РЕЖИМЫ (FLY И СТУПЕНЧАТЫЙ ТП)
                ...
                -- ТОТ САМЫЙ АФК КУСТОВОЙ ГЛИТЧ (Пассивное золото без смертей)
                elseif getgenv().TreasureAutoFarm.Mode == "AFK" then
                    -- Переносим тебя под кусты справа от Stage 1 (Водопадная зона)
                    hrp.CFrame = CFrame.new(-55, -9, -360)
                    
                    -- Создаем невидимый жесткий плот, игра думает, что мы плывем по локации
                    ...
                    -- Просто сидим тут, пока работает режим
