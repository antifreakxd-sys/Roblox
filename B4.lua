...
ScreenGui.Name = "AntiFreak_V14_Core"
...
-- ЖЕСТКИЙ NOCLIP + СБРОС ИНЕРЦИИ (Защита от вылета за карту на 1500 скорости)
...
-- ИСПРАВЛЕННЫЙ FLY С КРАТКОВРЕМЕННЫМ АНКЕРОМ (100% зачет зон на 1500)
local function safeFlyTo(targetPos)
    ...
    if hrp and getgenv().TreasureAutoFarm.Mode == "Fly" then 
        -- Намертво стопорим тело прямо внутри триггера, чтобы сервер успел его обсчитать
        hrp.CFrame = CFrame.new(targetPos)
        hrp.Anchored = true
        -- Динамическая пауза: на 1500 скорости стоим чуть дольше, чтобы не было пропусков голды
        local delayTime = (getgenv().TreasureAutoFarm.FlySpeed >= 1200) and 0.45 or 0.2
        task.wait(delayTime)
        hrp.Anchored = false
    end
end

-- ИСПРАВЛЕННЫЙ СТУПЕНЧАТЫЙ ТЕЛЕПОРТ ПОД КАРТОЙ (100% стабильность)
local function safeTPTo(targetPos)
    ...
    local steps = math.ceil(distance / 90) -- Снизили шаг до 90 для надежной подгрузки чанков
    ...
    while true do
        ...
        if getgenv().TreasureAutoFarm.Mode ~= "None" and hrp then
            pcall(function()
                ...
                -- ТОТ САМЫЙ РАБОЧИЙ АФК-ГЛИТЧ В КУСТАХ ПОД 1 ЗОНОЙ
                elseif getgenv().TreasureAutoFarm.Mode == "AFK" then
                    -- Четкие координаты кустов/водопада на стыке первой зоны
                    ...
                    -- Замораживаем персонажа в кустах, чтобы золото капало на пассиве
                    hrp.Anchored = true
