...
ScreenGui.Name = "AntiFreak_V23_Farm"
...
Title.Text = "by @AntiFreakXD" -- Исправлено по твоей просьбе, только твой ник!
...
-- НАДЁЖНЫЙ NOCLIP (Сброс векторов движения для полной ликвидации инерции)
...
-- ОБНОВЛЕННАЯ ФУНКЦИЯ ДВИЖЕНИЯ FARM (Никакого зацикливания на одной точке)
local function safeFarmTo(targetPos, stageIndex)
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end

    -- Движение чистым инкрементом векторов CFrame
    ...
    if hrp and getgenv().TreasureAutoFarm.Mode == "Fly" then 
        ...
        hrp.Anchored = true -- Жестко фиксируем тело в точке назначения
        
        -- Спам пакетов касания (Пробиваем хитбокс зоны на сервере)
        if stageIndex then
            pcall(function()
...
task.spawn(function()
    while true do
        ...
        -- Контроль полной готовности персонажа к спавну
        if getgenv().TreasureAutoFarm.Mode == "Fly" and hrp and hum and hum.Health > 0 then
            pcall(function()
                ...
                -- ИСПРАВЛЕНО: Чёткое линейное следование по стадиям 2 - 10
                for i = 2, 10 do
                    ...
                    -- Если стадия загружена в Workspace — летим строго на её координаты. Никаких цикличных формул!
                    if stage and stage:FindFirstChild("DarknessPart") then 
                        centerPos = Vector3.new(stage.DarknessPart.Position.X, 85, stage.DarknessPart.Position.Z) 
                    else
                        -- Строгий резервный вектор, если StreamingEnabled скрыл чанк
                        centerPos = Vector3.new(-50, 85, -1025 + (i * -775))
                    end
                    
                    safeFarmTo(centerPos, i)
                    
                    -- Зигзаг-смещение для зачёта скрытых монетных зон
                    if i == 3 or i == 6 or i == 9 or i == 10 then
                        safeFarmTo(centerPos + Vector3.new(20, 0, 0), false)
                        safeFarmTo(centerPos - Vector3.new(20, 0, 0), false)
                        safeFarmTo(centerPos, i)
                    else
                        task.wait(0.05)
                ...
                -- Влёт на Золотой Сундук в финал
                ...
                -- Чистый и моментальный сброс для старта следующего круга
                if getgenv().TreasureAutoFarm.Mode == "Fly" then
