...
ScreenGui.Name = "AntiFreak_V24_Stable"
...
FlyToggleBtn.Text = "🚀 LAUNCH FLY FARM"
...
UI.FlyToggleBtn.MouseButton1Click:Connect(function()
    if getgenv().TreasureAutoFarm.Mode == "Fly" then 
        ...
        UI.FlyToggleBtn.Text = "🚀 LAUNCH FLY FARM"
        ...
        UI.FlyToggleBtn.Text = "⚡ FLY FARM: ACTIVE" 
...
-- ТОТАЛЬНЫЙ CFRAME NOCLIP (Стирает инерцию каждую миллисекунду, спасает от кика за скорость)
...
-- ФУНКЦИЯ ПЕРЕМЕЩЕНИЯ (Микро-прыжки на СFrame с предзагрузкой чанков и 25-кратным спамом тачей)
local function safeFarmTo(targetPos, stageIndex)
    ...
    while getgenv().TreasureAutoFarm.Mode == "Fly" and (hrp.Position - targetPos).Magnitude > 15 do
        ...
        -- Пошагово режем путь на мелкие куски по 80 единиц
        local stepSize = 80
        local nextPos = currentPos + (targetPos - currentPos).Unit * math.min(stepSize, distance)
        
        -- Обходим StreamingEnabled: принудительно грузим чанки вокруг следующего шага
        if LocalPlayer.RequestStreamAroundAsync then
            pcall(function() LocalPlayer:RequestStreamAroundAsync(nextPos) end)
        end
    ...
    if hrp and getgenv().TreasureAutoFarm.Mode == "Fly" then 
        ...
        hrp.Anchored = true -- Намертво якорим торс внутри триггера стадии
        
        -- РЕШЕНИЕ ПРОБЛЕМЫ ПРОПУСКОВ: Выстреливаем 25 пакетов касания за одну миллисекунду!
        if stageIndex then
            pcall(function()
        ...
        -- Динамический таймер фиксации (на скорости 1500 стоим чуть дольше, чтобы сервер успел выдать монеты)
        local fixTime = (getgenv().TreasureAutoFarm.FlySpeed >= 1200) and 0.55 or 0.25
        task.wait(fixTime)
        hrp.Anchored = false
    end
end
...
task.spawn(function()
    while true do
        ...
        -- ФИКС СПАВН-КИЛЛА: Скрипт не тронет тебя, пока ХП полностью не прогрузится на спавне
        if getgenv().TreasureAutoFarm.Mode == "Fly" and hrp and hum and hum.Health > 0 then
            pcall(function()
                ...
                -- Поднимаем безопасную высоту до Y = 120 (над всеми текстурами гор и пещер)
                local startPos = Vector3.new(-50, 120, -1025)
                local st1 = normalStages:FindFirstChild("CaveStage1")
                if st1 and st1:FindFirstChild("DarknessPart") then 
                    startPos = Vector3.new(st1.DarknessPart.Position.X, 120, st1.DarknessPart.Position.Z) 
                    pcall(function() 
                        for i = 1, 10 do
                ...
                -- Строгое линейное движение по стадиям 2 - 10 из Workspace
                for i = 2, 10 do
                    ...
                    if stage and stage:FindFirstChild("DarknessPart") then 
                        centerPos = Vector3.new(stage.DarknessPart.Position.X, 120, stage.DarknessPart.Position.Z) 
                    else
                        centerPos = Vector3.new(-50, 120, -1025 + (i * -775))
                    end
                    
                    safeFarmTo(centerPos, i)
                    
                    -- Двойные зигзаги для зачета скрытых монетных зон игры
                    if i == 3 or i == 6 or i == 9 or i == 10 then
                        safeFarmTo(centerPos + Vector3.new(15, 0, 0), false)
                        safeFarmTo(centerPos - Vector3.new(15, 0, 0), false)
                        safeFarmTo(centerPos, i)
                    else
                        task.wait(0.02)
                ...
                -- Моментальный ресет персонажа для мгновенного перезапуска круга
                if getgenv().TreasureAutoFarm.Mode == "Fly" then
