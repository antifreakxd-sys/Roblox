...
-- ТОТАЛЬНЫЙ NOCLIP + ПОЛНАЯ БЛОКИРОВКА ЭФФЕКТОВ ВОДЫ
RunService.Stepped:Connect(function()
    if getgenv().TreasureAutoFarm.Mode == "Fly" and LocalPlayer.Character then
        local char = LocalPlayer.Character
        
        -- Убираем состояния урона и плавания, чтобы океан не дамажил сквозь пустые чанки
        ...
        for _, part in pairs(char:GetDescendants()) do
            ...
            -- Удаляем динамические триггеры урона воды вокруг персонажа
...
-- Умная функция полета чистым CFrame вектором
local function safeFarmTo(targetPos, stageIndex)
    ...
    -- РЕШЕНИЕ: До 1000 — летим внутри пещер (85), на 1000+ — уходим в тоннель над водой (52)
    local calculatedY = (currentSpeed >= 1000) and 52 or 85
    ...
    while getgenv().TreasureAutoFarm.Mode == "Fly" and (hrp.Position - dynamicTarget).Magnitude > 15 do
        ...
        calculatedY = (currentSpeed >= 1000) and 52 or 85
    ...
    if hrp and getgenv().TreasureAutoFarm.Mode == "Fly" then 
        ...
        hrp.Anchored = true -- Фиксируем тело
        ...
        if stageIndex then
            pcall(function()
                ...
                if darkness then
                    -- Если скорость 1000+, зажимаем сервер спамом пакетов до победного
                    local loops = (currentSpeed >= 1000) and 25 or 5
                    for i = 1, loops do
            ...
            -- Тайм-фиксатор под высокую скорость
...
task.spawn(function()
    while true do
        ...
        -- Ждем полной готовности ХП на спавне
        if getgenv().TreasureAutoFarm.Mode == "Fly" and hrp and hum and hum.Health > 0 then
            pcall(function()
                ...
                local currentSpeed = getgenv().TreasureAutoFarm.FlySpeed or 220
                local startY = (currentSpeed >= 1000) and 52 or 85
                ...
                for i = 2, 10 do
                    ...
                    -- ЗИГЗАГ: Работает только на нормальной скорости меньше 1000 (высота 85)
                    currentSpeed = getgenv().TreasureAutoFarm.FlySpeed or 220
                    if currentSpeed < 1000 and (i == 3 or i == 6 or i == 9 or i == 10) then
                        safeFarmTo(Vector3.new(stagePos.X + 18, 85, stagePos.Z), false)
                        safeFarmTo(Vector3.new(stagePos.X - 18, 85, stagePos.Z), false)
                        safeFarmTo(Vector3.new(stagePos.X, 85, stagePos.Z), i)
                    end
                end
