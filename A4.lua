...
-- ТОТАЛЬНЫЙ СБРОС ФИЗИКИ (Убираем инерцию торса намертво)
...
-- Функция Блинка под землей с выходом в триггер
local function blinkFarmTo(targetPos, stageIndex)
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end

    -- ШАГ 1: Мгновенный блинк ПОД землю (Y = -25, тут нет триггеров ебучей воды)
    local undergroundPos = Vector3.new(targetPos.X, -25, targetPos.Z)
    hrp.CFrame = CFrame.new(undergroundPos)
    RunService.Heartbeat:Wait()

    if getgenv().TreasureAutoFarm.Mode == "Fly" and stageIndex then
        -- ШАГ 2: Выныриваем ровно вверх внутрь триггера стадии (Y = 85)
        local triggerPos = Vector3.new(targetPos.X, 85, targetPos.Z)
        hrp.CFrame = CFrame.new(triggerPos)
        hrp.Anchored = true -- Замораживаем тело, чтобы сервер успел обсчитать хитбокс
        
        -- Шлем пакеты прямо внутри триггера
        pcall(function()
        ...
        -- Адаптивная задержка под скорость (на 1500 стоим чуть дольше, чтобы голда прилетела 100%)
        local currentSpeed = getgenv().TreasureAutoFarm.FlySpeed or 220
        local fixTime = (currentSpeed >= 1200) and 0.5 or 0.25
        task.wait(fixTime)
        
        -- ШАГ 3: Мгновенно ныряем обратно под землю перед следующим блинком
        hrp.Anchored = false
        hrp.CFrame = CFrame.new(undergroundPos)
        RunService.Heartbeat:Wait()
    end
...
task.spawn(function()
    while true do
        ...
        -- Контроль ХП на спавне (защита от бесконечных смертей)
        if getgenv().TreasureAutoFarm.Mode == "Fly" and hrp and hum and hum.Health > 0 then
            pcall(function()
                ...
                -- Сходу ныряем под спавн на безопасную глубину
                hrp.CFrame = CFrame.new(-50, -25, -1025)
                task.wait(0.4)

                -- Блинкаемся под стадии со 1 по 10
                for i = 1, 10 do
                    ...
                    blinkFarmTo(stagePos, i)
                    
                    -- На мелкой скорости делаем зигзаг, на 1000+ блинкаемся строго по центру напрямую
                    local currentSpeed = getgenv().TreasureAutoFarm.FlySpeed or 220
                    if currentSpeed < 1000 and (i == 3 or i == 6 or i == 9 or i == 10) then
                        blinkFarmTo(Vector3.new(stagePos.X + 20, 85, stagePos.Z), false)
                        blinkFarmTo(Vector3.new(stagePos.X - 20, 85, stagePos.Z), false)
                        blinkFarmTo(stagePos, i)
                    end
                end
                
                -- Прыжок на финальный Золотой Сундук
                ...
                -- Моментальное самоубийство для перезапуска цикла
                if getgenv().TreasureAutoFarm.Mode == "Fly" then
