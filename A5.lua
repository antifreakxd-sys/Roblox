...
-- ВЕЧНЫЙ NOCLIP (Стирает инерцию в ноль каждую миллисекунду, спасает от кика за скорость)
...
-- ДВИЖЕНИЕ НА CFRAME С АДАПТИВНОЙ ВЫСОТОЙ ПОД СКОРОСТЬ
local function safeFarmTo(targetPos, stageIndex)
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end

    -- Авто-высота: до 1000 летим в пещере (85), на 1000+ уходим в космос от воды и лагов (650)
    local currentSpeed = getgenv().TreasureAutoFarm.FlySpeed or 220
    local calculatedY = (currentSpeed >= 1000) and 650 or 85
    local dynamicTarget = Vector3.new(targetPos.X, calculatedY, targetPos.Z)

    while getgenv().TreasureAutoFarm.Mode == "Fly" and (hrp.Position - dynamicTarget).Magnitude > 15 do
        if not LocalPlayer.Character or not hrp.Parent then break end
        
        -- Пересчет высоты на ходу, если дернули слайдер в процессе полета
        currentSpeed = getgenv().TreasureAutoFarm.FlySpeed or 220
        calculatedY = (currentSpeed >= 1000) and 650 or 85
        dynamicTarget = Vector3.new(targetPos.X, calculatedY, targetPos.Z)
    ...
    if hrp and getgenv().TreasureAutoFarm.Mode == "Fly" then 
        ...
        hrp.Anchored = true -- Якорим торс в триггере
        
        -- Сверхпробитие чекпоинтов пакетами
        if stageIndex then
            pcall(function()
                local darkness = Workspace.BoatStages.NormalStages["CaveStage" .. tostring(stageIndex)]:FindFirstChild("DarknessPart")
                if darkness then
                    local loops = (currentSpeed >= 1200) and 20 or 5
                    for i = 1, loops do
            ...
            -- Таймер удержания под высокую скорость
            local fixTime = (currentSpeed >= 1000) and 0.45 or 0.15
            task.wait(fixTime)
        end
        hrp.Anchored = false
    end
...
task.spawn(function()
    while true do
        ...
        -- Защита от спавн-килла (ждем ХП персонажа)
        if getgenv().TreasureAutoFarm.Mode == "Fly" and hrp and hum and hum.Health > 0 then
            pcall(function()
                ...
                -- Выставляем стартовую высоту по скорости
                local currentSpeed = getgenv().TreasureAutoFarm.FlySpeed or 220
                local startY = (currentSpeed >= 1000) and 650 or 85
                ...
                for i = 2, 10 do
                    ...
                    -- Зигзаг-смещение (только для малых скоростей в пещере, в облаках на 1000+ оно не нужно)
                    currentSpeed = getgenv().TreasureAutoFarm.FlySpeed or 220
                    if currentSpeed < 1000 and (i == 3 or i == 6 or i == 9 or i == 10) then
                        local currentY = (currentSpeed >= 1000) and 650 or 85
                        safeFarmTo(Vector3.new(stagePos.X + 15, currentY, stagePos.Z), false)
                        safeFarmTo(Vector3.new(stagePos.X - 15, currentY, stagePos.Z), false)
                        safeFarmTo(Vector3.new(stagePos.X, currentY, stagePos.Z), i)
                    end
                end
                
                -- Влет на Золотой Сундук
                ...
                -- Моментальный ресет для перезапуска бесконечного цикла
                if getgenv().TreasureAutoFarm.Mode == "Fly" then
