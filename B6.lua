...
ScreenGui.Name = "AntiFreak_V12_Core"
...
-- Глобальный доступ для следующих частей
getgenv().AF_CoreUI = {
...
if not UI then return warn("Сначала запустите Часть 1!") end
...
-- Защита от AFK-кика Roblox (20 минут)
pcall(function()
    for _, v in pairs(getconnections(LocalPlayer.Idled)) do v:Disable() end
end)

-- Noclip + Сброс физической инерции (защита от кика античитом за скорость)
...
-- Алгоритм безопасного полета с динамической задержкой
local function safeFlyTo(targetPos)
    local char = LocalPlayer.Character
    if not char or not char:FindFirstChild("HumanoidRootPart") then return end
    local hrp = char.HumanoidRootPart

    while getgenv().TreasureAutoFarm.Mode == "Fly" and (hrp.Position - targetPos).Magnitude > 8 do
    ...
    if hrp and getgenv().TreasureAutoFarm.Mode == "Fly" then 
        ...
        local factor = math.clamp(getgenv().TreasureAutoFarm.FlySpeed / 500, 0.25, 0.8)
        task.wait(factor) 
    end
end
    ...
    while true do
        ...
        if getgenv().TreasureAutoFarm.Mode ~= "None" and hrp then
            pcall(function()
                ...
                -- РЕЖИМ 1 И 2: Fly Farm / Обычный TP Farm
                    ...
                    for i = 2, 10 do
                        ...
                        if getgenv().TreasureAutoFarm.Mode == "Fly" then
                            ...
                            if i == 3 or i == 6 or i == 9 or i == 10 then
                                ...
                                safeFlyTo(centerPos - Vector3.new(15, 0, 0))
                                safeFlyTo(centerPos)
                                task.wait((getgenv().TreasureAutoFarm.FlySpeed > 1000) and 0.95 or 0.65)
                            else
                                task.wait(0.4)
                            end
                ...
                -- РЕЖИМ 3: Настоящий Сверхбыстрый цикличный AFK Farm
                elseif getgenv().TreasureAutoFarm.Mode == "AFK" then
                    for i = 1, 10 do
                        if getgenv().TreasureAutoFarm.Mode ~= "AFK" then break end
                        local centerPos = Vector3.new(-50, 85, -1025 + (i * -775))
                        local stage = normalStages:FindFirstChild("CaveStage" .. i)
                        if stage and stage:FindFirstChild("DarknessPart") then centerPos = Vector3.new(stage.DarknessPart.Position.X, 85, stage.DarknessPart.Position.Z) end
                ...
                -- ФИНАЛ: Забираем золото из сундука
                ...
                -- РЕСПАВН (для запуска следующего бесконечного круга)
                if getgenv().TreasureAutoFarm.Mode ~= "None" then
                    local respawned = false
                    local conn
                    conn = LocalPlayer.CharacterAdded:Connect(function() respawned = true end)
