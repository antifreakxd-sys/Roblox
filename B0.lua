...
ScreenGui.Name = "AntiFreak_V21_Fixed"
...
Title.Text = "🌌 ANTIFREAK INFINITY v21"
...
-- ЧИСТЫЙ NOCLIP (Каждую миллисекунду полностью очищает инерцию торса)
...
-- ИСПРАВЛЕННЫЙ FLY (Камера больше не ломается, чанки грузятся пошагово)
local function godFlyTo(targetPos, stageIndex)
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end

    -- Полет строго через прямую математическую перезапись CFrame векторов
    ...
    if hrp and getgenv().TreasureAutoFarm.Mode == "Fly" then 
        ...
        -- Сверхпробитие триггера: спамим сервер 10 раз за сотую долю секунды
        if stageIndex then
            pcall(function()
        ...
        -- Оптимальное время фиксации для 1500 скорости
...
task.spawn(function()
    while true do
        ...
        -- ФИКС БЕСКОНЕЧНЫХ СМЕРТЕЙ: Ждем полной прогрузки здоровья персонажа перед стартом телепорта
        if getgenv().TreasureAutoFarm.Mode == "Fly" and hrp and hum and hum.Health > 0 then
            pcall(function()
                ...
                for i = 2, 10 do
                    ...
                    -- Двойной зигзаг для зачета триггеров
