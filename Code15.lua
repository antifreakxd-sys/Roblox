...
getgenv().TreasureAutoFarm = {
    Enabled = false,
    FlySpeed = 100,        -- Начальная скорость
    WaitAtChest = 5        -- Ожидание у сундука
}
...
ScreenGui.Name = "BABFT_Ultimate_Farm"
...
Title.Text = "BUILD A BOAT FARM v2"
...
SliderButton.Position = UDim2.new(0.33, -11, -0.25, 0)
SliderButton.BackgroundColor3 = Color3.fromRGB(0, 150, 255)
SliderButton.Text = ""
UICorner_SliderBtn.CornerRadius = UDim.new(1, 0)

-- УНИВЕРСАЛЬНАЯ ФУНКЦИЯ ПЕРЕТАСКИВАНИЯ (Для Кнопки и для Меню)
...
-- ЛОГИКА ПОЛЗУНКА СКОРОСТИ
local minSpeed, maxSpeed = 40, 220
...
-- НАДЕЖНЫЙ ДВИЖОК ПОЛЁТА (БЕЗ TWEEN)
local function safeFlyTo(targetPos)
    local character = LocalPlayer.Character
    if not character or not character:FindFirstChild("HumanoidRootPart") then return end
    local hrp = character.HumanoidRootPart

    -- Жесткое создание платформы под ногами персонажа
    ...
    while getgenv().TreasureAutoFarm.Enabled and (hrp.Position - targetPos).Magnitude > 5 do
        ...
        -- Вычисление направления и перемещение покадрово
        local dir = (targetPos - hrp.Position).Unit
        local nextPos = hrp.Position + dir * (getgenv().TreasureAutoFarm.FlySpeed * RunService.Heartbeat:Wait())
        
        hrp.CFrame = CFrame.new(nextPos, targetPos)
        hrp.Velocity = Vector3.new(0,0,0) -- Гасим физику анти-чита
    ...
    while true do
        ...
        if getgenv().TreasureAutoFarm.Enabled then
            pcall(function()
                ...
                -- Базовые векторы стадий на случай если они НЕ ПРОГРУЗИЛИСЬ
                -- Игра использует фиксированное смещение по оси Z
                local startZ = -250
                local stepZ = -775 -- Среднее расстояние между центрами стадий
                
                for i = 1, 10 do
                    if not getgenv().TreasureAutoFarm.Enabled then break end
                    
                    local calculatedPos = Vector3.new(-50, 55, startZ + (i * stepZ))
                    
                    -- Если стадия прогружена — берем её точные координаты
                    ...
                    if darknessPart then
                        calculatedPos = darknessPart.Position
                    end
                    
                    -- Летим к точке
                    safeFlyTo(calculatedPos)
                    task.wait(0.4) -- Задержка на чекпоинте для 100% зачисления сервером
                end
                ...
                if getgenv().TreasureAutoFarm.Enabled and normalStages and normalStages:FindFirstChild("TheEnd") then
                    ...
                    if trigger then
                        safeFlyTo(trigger.Position)
                    else
                        safeFlyTo(Vector3.new(-50, -15, -9200)) -- Точные координаты финала, если не прогрузился
                    end
